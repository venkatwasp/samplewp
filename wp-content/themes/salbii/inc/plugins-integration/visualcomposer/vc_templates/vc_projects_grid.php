...
$title = $grid_columns_count = $grid_teasers_count = $grid_layout = $grid_project_taxonomy = $grid_link = $grid_link_target = '';
$grid_template = $grid_thumb_size = $grid_posttypes = $grid_layout_mode = $grid_taxomonies = $grid_categories = $posts_in = $posts_not_in = '';
$grid_content = $el_class = $width = $orderby = $order = $el_position = $isotope_item = $hover_effect = '';

extract(shortcode_atts(array(
	'title' => '',
	'grid_columns_count' => 4,
	'grid_teasers_count' => 'All',
	...
	'orderby' => NULL,
	'order' => 'DESC',
	'hover_effect' => 'teasers-hover-effect-1',
	'el_position' => ''
), $atts));

// set thumbnail size based on selected number of grid columns
if ( $grid_layout_mode != 'masonry' ) {
	switch ($grid_columns_count) {
		case 4:
			$grid_thumb_size = '390x390';
			break;
		case 3:
			$grid_thumb_size = '487x487';
			break;
		case 2:
			$grid_thumb_size = '642x642';
			break;
		case 1:
...
while ( $my_query->have_posts() ) {
	...
	$categories_css = '';
	$project_types = array();
	if( $grid_template == 'filtered_grid' || $grid_project_taxonomy != '' ) {
		...
		if(!is_wp_error($post_categories)) {
			foreach($post_categories as $cat) {
				...
				$project_types[] = esc_attr($cat->name);
			}
		}
	...
	// Get custom project card colors if any defined in metaboxes
	// Backrgound and text color metaboxed defined in plugins/lbmn-projects/lbmn-projects.php
	$project_card_bgcolor = rwmb_meta( 'lbmn_project_bgcolor', array(), $post_id );
	$project_card_textcolor = rwmb_meta( 'lbmn_project_textcolor', array(), $post_id );
	$card_style = $card_text_style = '';

	if ( isset( $project_card_bgcolor ) && $project_card_bgcolor != '' ) {
		$card_style = ' style="background-color:'.$project_card_bgcolor.'};"';
	}
	if ( isset( $project_card_textcolor ) && $project_card_textcolor != '' ) {
		$card_text_style = ' style="color:'.$project_card_textcolor.'};"';
	}

	// Preprate projec types for output if it's set to display project types
	$project_types_string = '';
	if ( $grid_project_taxonomy != '' ) {
		$project_types_string = implode(", ", $project_types);
	}

	$teasers .= '<li class="'.$isotope_item.apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, $li_span_class, 'vc_teaser_grid_li').$categories_css.'"'.$card_style.'>';
	$teasers .= '<div class="isotope-item_inner-wrapper">';
	// If grid layout is: Title + Thumbnail + Text
	if ( $grid_layout == 'title_thumbnail_text' ) {
		if ( $post_title ) 	{
			$to_filter = '<h2 class="post-title"'.$card_text_style.'>' . $link_title_start . $post_title . $link_title_end . '</h2>';
		...
		if ( $project_types_string ) {
			$to_filter = '<p class="post-categories"'.$card_text_style.'>' . $project_types_string . '</p>';
			$teasers .= apply_filters('vc_teaser_grid_type', $to_filter, array("grid_layout" => $grid_layout, "ID" => $post_id, "types" => $project_types_string, "media_link" => $p_link) );
		}
		if ( $content ) {
			$to_filter = '<div class="entry-content"'.$card_text_style.'>' . $content . '</div>';
	...
	// If grid layout is: Thumbnail + Title + Text
		...
		if ( $project_types_string ) {
			$to_filter = '<p class="project-types"'.$card_text_style.'>' . $project_types_string . '</p>';
			$teasers .= apply_filters('vc_teaser_grid_type', $to_filter, array("grid_layout" => $grid_layout, "ID" => $post_id, "types" => $project_types_string, "media_link" => $p_link) );
		}
		if ( $content ) {
			$to_filter = '<div class="entry-content"'.$card_text_style.'>' . $content . '</div>';
	...
	*/
	$teasers .= $link_image_start . $link_image_end; // make whole card clikable
	$teasers .= '</div>';//inner wrapper
...
if ( $teasers ) { $teasers = '<div class="teaser_grid_container">'.$categories_list_output.'<ul class="masonry-teasers-container wpb_thumbnails wpb_thumbnails-fluid clearfix" data-layout-mode="'.$grid_layout_mode.'">'. $teasers .'</ul></div>'; }
...
if ( $hover_effect ) {
	$grid_class .= ' ' . $hover_effect;
}
$css_class =  apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, 'wpb_teaser_grid wpb_content_element columns masonry-grid '.$grid_class.$width.$el_class, $this->settings['base']);

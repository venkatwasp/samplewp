...
class phpColors_Color {
	...
	public function getCssRgba($transparency = 1) {
		// convert to string
		$transparency = (string)$transparency;

		// $transparency has format 0.XX leave it as it is otherwise convert from % into 0.XX format
		if ( !( stristr($transparency, '.') || $transparency == '1' ) ) {
			$transparency = str_replace('%', '', $transparency); // remove % char
			$transparency = '.' . $transparency; // convert from % into 0.XX format
		}
